<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ApproveRecipe.aspx.cs"
    MasterPageFile="/Umbraco/masterpages/umbracoPage.Master" Inherits="Hershey.Web.UmbracoAddons.Moderation.Recipes.ApproveRecipe" %>

<%@ Register TagPrefix="cc1" Namespace="umbraco.uicontrols" Assembly="controls" %>
<%@ Register Src="~/usercontrols/recipes/Details/Kitchens.ascx" TagName="Details"
    TagPrefix="Recipe" %>
<%@ Register Src="RecipeReviews.ascx" TagName="RecipeReviews" TagPrefix="uc1" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="body" runat="server">
            ...
            <asp:Repeater runat="server" ID="rptReviews">
                <ItemTemplate>
                    <li>
                        <p class="group_wrap">
                            <span class="approved">
                                <label for="cbRating<%# Eval("RecipeRatingsID") %>">
                                    Approved</label>
                                <input type="checkbox" name="cbRating<%# Eval("RecipeRatingsID") %>" id="cbRating<%# Eval("RecipeRatingsID") %>"
                                    <%# IsChecked(Container) %> />
                            </span><span class="username">
                                <%# Eval("UserName") %></span> <span class="tip-date r">
                                    <%# Eval("Created")%>
                        </p>
                        <p>
                            <%# Eval("Comment")%>
    ...
    <cc1:Pane ID="pane_Pending" runat="server" Visible="false">
        <%--Moderate Reviews--%>
        <uc1:RecipeReviews ID="RecipeReviews1" runat="server" />
    </cc1:Pane>
</asp:Content>
