...
namespace System.Data
{
    public class SqlHelper
    {
        ...
        public static string ConnString
        {
            ...
            get
            {
                if (string.IsNullOrEmpty(connString))
                    connString = ConfigurationManager.ConnectionStrings["ECOM"].ConnectionString;
                return connString;
            }
        }
        ...
        public static t UpdateEntity<t>(t obj) where t : INotifyPropertyChanging, INotifyPropertyChanged
        {
            t tmp = default(t);
            try
            {
                using (SqlConnection conn = new SqlConnection(ConnString))
                {
                    ...
                    sql.AppendFormat("UPDATE {0} SET", objType.Name);
                    System.Reflection.PropertyInfo pk = null;
                    foreach (System.Reflection.PropertyInfo pi in objType.GetProperties())
                    {
                        object val = pi.GetValue(obj, null);
                        ColumnAttribute ca = pi.GetCustomAttributes(false).Where(a => a is ColumnAttribute).FirstOrDefault() as ColumnAttribute;                        

                        if (ca != null)
                        {
                            Params.Add(new SqlParameter("@" + pi.Name, val));

                            if (ca.IsPrimaryKey)
                                pk = pi;
                            else
                                sql.AppendFormat(" {0} = @{0},", pi.Name);
                        }
                    }
                    sql.Remove(sql.Length - 1, 1);
                    if (pk != null)
                    {
                        sql.AppendFormat(" WHERE {0}=@{0}", pk.Name);
                        using (SqlCommand com = new SqlCommand(sql.ToString(), conn))
                        {
                            com.CommandType = CommandType.Text;
                            if (Params != null && Params.Count > 0)
                                com.Parameters.AddRange(Params.ToArray());
