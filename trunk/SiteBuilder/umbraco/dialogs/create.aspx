  ...
  <script language="javascript" type="text/javascript">
			...
			function dialogHandler(id) {
				...
				if (id > 0) {
				  ...
				  //document.getElementById("pageName").innerHTML = "<p><strong><%=umbraco.ui.Text(umbraco.helper.Request("app"))%></strong> <%= umbraco.ui.Text("moveOrCopy","nodeSelected") %></p>";
					//document.getElementById("pageNameHolder").setAttribute("class", "success");
			...
			function updateName(result) {
				//document.getElementById("pageName").innerHTML = "<p><strong>" + result + "</strong> <%= umbraco.ui.Text("moveOrCopy","nodeSelected") %></p>";
				//document.getElementById("pageNameHolder").setAttribute("class", "success");
...
<asp:Content ContentPlaceHolderID="body" runat="server">
    ...
    <asp:Panel runat="server" ID="panel_buttons">
      ...
      <input type="button" id="ok" value="<%=umbraco.ui.Text("ok")%>" onclick="revertOpener(); document.location.href = 'create.aspx?nodeType=<%=umbraco.helper.Request("nodeType")%>&app=<%=umbraco.helper.Request("app")%>&nodeId=' + document.getElementById('nodeId').value"
        ...
        &nbsp; <em><%= umbraco.ui.Text("or") %></em>&nbsp; <a href="#" style="color: blue" onclick="top.closeModal()"><%=umbraco.ui.Text("cancel")%></a>
      </div>
    </asp:Panel>
