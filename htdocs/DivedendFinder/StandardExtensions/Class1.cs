...
namespace StandardExtensions
{
    ...
    {
        public static void blah<type>(this System.Data.Linq.ITable tab, List<type> entities)
        {
            var entity = entities.FirstOrDefault();

            System.Type baseType = entities.AsQueryable().ElementType;

            System.Reflection.PropertyInfo field = baseType.GetProperties().Where(p =>
                ((System.Data.Linq.Mapping.ColumnAttribute)p.GetCustomAttributes(false).FirstOrDefault()).IsPrimaryKey).FirstOrDefault();
            ConstantExpression value = Expression.Constant(field.GetValue(entity, null));

            ParameterExpression param = Expression.Parameter(baseType, "c");
            MemberExpression selector = Expression.Property(param, field);

            BinaryExpression filter = Expression.Equal(selector, value);
            BinaryExpression filter2 = Expression.And(filter, Expression.NotEqual(selector, value));

            LambdaExpression expr = Expression.Lambda(filter, param);

            System.Reflection.PropertyInfo dcProp = tab.AsQueryable().Provider.GetType().GetProperties()[0];
            DataContext dc = (DataContext)dcProp.GetValue(tab, null);


            var e = Expression.Call(typeof(Queryable), "Where",
                new Type[] { baseType }, Expression.Constant(dc.GetTable(baseType)), expr);
            IEnumerable query = tab.Provider.Execute<IEnumerable>(e);

            bool Exists = query.GetEnumerator().MoveNext();
            query.GetEnumerator().Reset();


            string a = "";
        }

        public static void SetTextThreadSafe(this System.Windows.Forms.Form form, System.Windows.Forms.Control control, string text)
        {
            if (control.InvokeRequired)
            {
                CustomExtensions.SetTextCallback c = new SetTextCallback(CustomExtensions.SetText);
                form.Invoke(c, new object[] { control, text });
            }
            else
            {
                CustomExtensions.SetText(control, text);
            }

        }
        public delegate void SetTextCallback(System.Windows.Forms.Control control, string text);
        public static void SetText(System.Windows.Forms.Control control, string text)
        {
            control.Text = text;
        }

        public static void DeleteAll(this System.Data.Linq.ITable tab)
        {
            try
            {
                using (SqlCommand com = new SqlCommand(string.Format("TRUNCATE TABLE {0}", new string[] { tab.ElementType.Name }), new System.Data.SqlClient.SqlConnection(tab.Context.Connection.ConnectionString)))
                {
                    com.Connection.Open();
                    com.ExecuteNonQuery();
            ...
            {
                using (SqlCommand com = new SqlCommand(string.Format("DELETE FROM {0}", new string[] { tab.ElementType.Name }), new System.Data.SqlClient.SqlConnection(tab.Context.Connection.ConnectionString)))
                {
                    com.Connection.Open();
                    com.ExecuteNonQuery();
                    com.Connection.Close();
        ...
        public static void InsertAll<t>(this System.Data.Linq.ITable tab, IEnumerable<t> collection)
        {
            using (System.Data.SqlClient.SqlBulkCopy bc = new System.Data.SqlClient.SqlBulkCopy(tab.Context.Connection.ConnectionString))
            {
                bc.BatchSize = 1000;
                bc.DestinationTableName = tab.ElementType.Name;
                bc.WriteToServer(collection.AsDataReader());
            }
        }
